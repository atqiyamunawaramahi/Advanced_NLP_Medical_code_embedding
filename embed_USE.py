...
# df
with open('/data1/mrahman/snomed_embeddings/snomed_embeddings/use.json') as f:
    use_emb = json.load(f)
# use_emb[14265004]
use_emb = {int(k):v for k,v in use_emb.items()}

def mean_emb(snmd_con):
    p = np.array([use_emb.get(x,[0]*512) for x in snmd_con])
...
embaded_snmd_use_arr = np.array(embaded_snmd.values.tolist())
embaded_snmd_use_arr = np.nan_to_num(embaded_snmd_use_arr)
# df
# embaded_snmd_n2v_arr

#random forest

X = embaded_snmd_use_arr
Y = df['outcome']
...
embed_model_lr = LogisticRegression(max_iter=100 ,C=1, verbose=1, n_jobs=-1).fit(X_train,y_train)
...
#SVM
embed_model_svm= svm.SVC(kernel='linear', verbose=1, max_iter=1000)
...
model.compile(loss='binary_crossen tropy', optimizer='adam', metrics=['accuracy'])
