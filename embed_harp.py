...
# df
with open('/data1/mrahman/snomed_embeddings/snomed_embeddings/harp.json') as f:
    harp_emb = json.load(f)
# harp_emb[14265004]
harp_emb = {int(k):v for k,v in harp_emb.items()}

def mean_emb(snmd_con):
    p = np.array([harp_emb.get(x,[0]*128) for x in snmd_con])
...
embaded_snmd_harp_arr = np.array(embaded_snmd.values.tolist())
embaded_snmd_harp_arr = np.nan_to_num(embaded_snmd_harp_arr)
# df
# embaded_snmd_n2v_arr

#random forest

X = embaded_snmd_harp_arr
Y = df['outcome']
