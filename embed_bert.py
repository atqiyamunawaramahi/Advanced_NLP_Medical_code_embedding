...
df['trazodone'].apply(lambda x:float(x)).sum()
...
# df
with open('/data1/mrahman/snomed_embeddings/bert.json') as f:
    bert_emb = json.load(f)
# bert_emb[14265004]
bert_emb = {int(k):v for k,v in bert_emb.items()}

def mean_emb(snmd_con):
    p = np.array([bert_emb.get(x,[0]*768) for x in snmd_con])
...
embaded_snmd_bert_arr = np.array(embaded_snmd.values.tolist())
embaded_snmd_bert_arr = np.nan_to_num(embaded_snmd_bert_arr)
# df
# embaded_snmd_n2v_arr

#random forest

X = embaded_snmd_bert_arr
Y = df['outcome']
...
#SVM
embed_model_svm= svm.SVC(kernel='linear', verbose=1, max_iter=10000)
