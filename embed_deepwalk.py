...
# df
with open('/data1/mrahman/snomed_embeddings/deepwalk.json') as f:
    deepwalk_emb = json.load(f)
# deepwalk_emb[14265004]
deepwalk_emb = {int(k):v for k,v in deepwalk_emb.items()}

def mean_emb(snmd_con):
    p = np.array([deepwalk_emb.get(x,[0]*128) for x in snmd_con])
...
embaded_snmd_deepwalk_arr = np.array(embaded_snmd.values.tolist())
embaded_snmd_deepwalk_arr = np.nan_to_num(embaded_snmd_deepwalk_arr)
df
embaded_snmd_n2v_arr

#random forest

X = embaded_snmd_deepwalk_arr
Y = df['outcome']
