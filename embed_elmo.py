...
# df
with open('/data1/mrahman/snomed_embeddings/snomed_embeddings/elmo.json') as f:
    elmo_emb = json.load(f)
# elmo_emb[14265004]
elmo_emb = {int(k):v for k,v in elmo_emb.items()}

def mean_emb(snmd_con):
    p = np.array([elmo_emb.get(x,[0]*1024) for x in snmd_con])
...
embaded_snmd_elmo_arr = np.array(embaded_snmd.values.tolist())
embaded_snmd_elmo_arr = np.nan_to_num(embaded_snmd_elmo_arr)
# df
# embaded_snmd_n2v_arr

#random forest

X = embaded_snmd_elmo_arr
Y = df['outcome']
